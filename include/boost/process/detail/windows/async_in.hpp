                         ...
                         ::boost::process::detail::windows::require_io_service
{
    ...
    {
        ::boost::detail::winapi::CloseHandle(pipe_->native_source());
    }
    ...
    {
        if (!pipe_)
            pipe_ = std::make_shared<boost::process::async_pipe>(get_io_service(exec.seq));

        ::boost::detail::winapi::HANDLE_ source_handle = std::move(*pipe_).source().native_handle();
